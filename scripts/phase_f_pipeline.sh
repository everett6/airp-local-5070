...
unload() { for u in http://127.0.0.1:11434 "$TUNED"; do
  curl -s "$u/api/generate" -d '{"model":"qwen3:8b","keep_alive":0}' >/dev/null || true; done; }
...
if [ ! -f results/walkforward_v6_fund_rank20d.json ]; then
  log "v6"; $PY -m app.sandbox.walkforward --config configs/v6_fund_rank20d.toml --force --ollama-url "$TUNED"
fi
...
if [ ! -f results/lap_v7_phase_f.jsonl ]; then
  log "LAP probe"; $PY -m app.sandbox.walkforward --config configs/v7_phase_f.toml --probe-lap
fi
if [ ! -f results/walkforward_v7_phase_f.json ]; then
  log "v7"; $PY -m app.sandbox.walkforward --config configs/v7_phase_f.toml
fi
unload
